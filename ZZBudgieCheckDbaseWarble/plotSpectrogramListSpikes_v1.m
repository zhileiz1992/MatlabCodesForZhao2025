function [fig_count]=plotSpectrogramListZZ_flexible_v1(ephys_struct, sound_field, fig_start, maxDur, plot_row, plot_col, save_folder, prefix, title_field, flim, clim, width, height, xgap, ygap, left_margin, bottom_margin, fig_pos, NFFT, windowSize, windowOverlap)
...
%   ax = gca;
  [ax, power, powerGrey, S, f, t]=showAudioSpectrogramZZ_flexible_v1(temp.(sound_field), temp.fs, gca(), flim, clim, NFFT, windowSize, windowOverlap);
  ...
  if ~isempty(spike_idx)
    ...
    xline(ax, spike_frame, 'w');
    % add a small text to show how many spikes are in the bursts
    text(ax, spike_frame(end), 7100, num2str(length(spike_frame)), 'Color', 'w', 'FontSize', 12);
  end
end
