...
function [fd_save_nc] = ZZ_exportIntanNC_v1(metaInfo, fd_save_nc, bird_adc_ch, partner_adc_ch, ephys_ch, acc_ch, prefix)
%% settings
...
parfor ri=1:length(metaInfo)
  ...
  for chunk_num = 1:ceil(metaInfo(ri).duration/chunk_dur)
    ...
    for ei=ephys_ch
      ...
      fn_nc = fullfile(fd_save_nc, sprintf('%s_chan%d', fn_prefix, ei));
      writer(fn_nc, ephys_data.timeVector, ephys_data.deltaT, ei, ephys_data.metaData, ephys_data.data, true);
    end
