...
% modified on 04/02/2024, export for a pair of birds at the same time
function [fds_save_nc] = ZZ_exportIntanNC_v3_pairs(warbleMetaAll, meta, pi, data_date, fd_base_save, prefix)
%% settings
...
for bi=1:length(meta.birdID{pi})
  if ~isempty(meta.birdID{pi}{bi})
    fd_save_this = fullfile(fd_base_save, meta.pairID{pi}, data_date, meta.birdID{pi}{bi}, 'NCFiles', prefix);
    if exist(fd_save_this, 'dir')
      rmdir(fd_save_this, 's');
    end
    mkdir(fd_save_this);
    fds_save_nc{bi} = fd_save_this;
  end
end

%% go through each row of the metaInfo struct
% ri = 2;
metaInfo = warbleMetaAll{pi};
parfor ri=1:length(metaInfo)
  ...
  % loop through two birds in a pair, save audio data for focal bird
  for bi=1:length(meta.birdID{pi})
    if isempty(meta.ephys_ch_list{pi}{bi})
      continue;
    end
    bird_adc_ch = meta.audio_ch_intan(pi,bi);
    partner_adc_ch = setdiff(meta.audio_ch_intan(pi,:), bird_adc_ch);
    ephys_ch = meta.ephys_ch_list{pi}{bi};
    acc_ch = meta.acc_ch_list{pi}{bi};
    % save into different folders for each bird
    fd_save_nc = fds_save_nc{bi};
...
%       song_data.metaData = sprintf('%s\t%d\t%d\t%d', fns_rhd(1).name, chunk_num, chunk_start, chunk_end);
      tempInfo = strsplit(strrep(fns_rhd(1).name, '.rhd', ''), '_');
      song_data.metaData = sprintf('%s_%s_%d_%d_%d', tempInfo{end-1}, tempInfo{end}, chunk_num, chunk_start, chunk_end);
      ...
      % save to nc files
      fn_prefix = sprintf('%s_%05d_%05d_%d_%s_%s', prefix, ri, chunk_num, sing_bird, meta.birdID{pi}{bi}, strrep(fns_rhd(1).name,'.rhd',''));
      ...
      % add a channel for the playback copy: 11-03-2024 added
      playback_chan = 22;
      playback_data = song_data;
      playback_ch_intan = meta.playback_ch_box(pi);
      playback_data.data = audio_all(playback_ch_intan, chunk_start:chunk_end);
      fn_nc = fullfile(fd_save_nc, sprintf('%s_chan%d', fn_prefix, playback_chan));
      writer(fn_nc, playback_data.timeVector, playback_data.deltaT, playback_chan, playback_data.metaData, playback_data.data, true);
    end
  end
end
end
