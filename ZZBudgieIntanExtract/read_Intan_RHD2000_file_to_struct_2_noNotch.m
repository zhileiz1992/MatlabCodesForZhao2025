function data = read_Intan_RHD2000_file_to_struct_2_noNotch(path, file, verbose)
...
%
% Modified April 2021, Brian Kardon
% Instead of being exported into base workspace, extracted information is stored
%   in the 'data' output struct. To see what information is available, try
%
%   > data = read_Intan_RHD2000_file(path, file)
%   > fieldnames(data)
%
% Access the data like so:
%
%   > plot(data.t_amplifier, data.amplifier_data(1,:))

% Zhilei modified: comment out the notch portion, since it's slow and we
% only care about the board_adc channels
...
if (num_aux_input_channels > 0)
    ...
    if (data_present)
        data.aux_input_data = aux_input_data;
        data.t_aux_input = t_aux_input;
    end
end
if (num_supply_voltage_channels > 0)
    data.supply_voltage_channels = supply_voltage_channels;
    if (data_present)
        data.supply_voltage_data = supply_voltage_data;
        data.t_supply_voltage = t_supply_voltage;
    end
end
if (num_board_adc_channels > 0)
...
if (num_temp_sensor_channels > 0)
    if (data_present)
        data.temp_sensor_data = temp_sensor_data;
        data.t_temp_sensor = t_temp_sensor;
    end
end
