function [warbleMetaAll, otherMetaAll] = ZZ_IdentifyWarbleFromIntan_v8_forPair1(fd_base_intan, fd_base_save, data_date, meta)
...
parfor batchIdx=1:length(fileStructSplit)
  ...
  for i = 1:length(fileStruct)
    ...
    fd_save_speaker = fullfile(fd_base_save, meta.pairID{2}, data_date, 'SpeakerTrigger');
...
fn_mat = fullfile(fd_base_save, meta.pairID{2}, data_date, 'SpeakerTrigger', sprintf('%s_%s.flatness.mat', meta.expID, data_date));
save(fn_mat, 'flatnessAll');

% remove electro_gui path to avoid conflict on the findpeaks function
clearPaths = {'/mnt/z4/zz367/LabSoftware/ElectroGui-main', '/mnt/z4/zz367/LabSoftware/ElectroGui', '/home/zz367/LabSoftware/ElectroGui'};
for ci=1:length(clearPaths)
  if ismember(clearPaths{ci}, strsplit(path,':'))
    rmpath(genpath(clearPaths{ci}));
  end
end
