% try different parameters for XGBoost modeling
% Different window size when averaging spike firing rate
...
fd_save_base = fullfile(fd_base, 'Figures', pairID, 'CompoundSyl', birdID, 'ensembleModel', 'paramSearch');



%% 1. Use the example neuron, try different lags
...
% try different win_size
win_list = [3, 5, 10, 15, 20, 25];
hop_list = [2, 3, 5, 5, 5, 5];
fd_save_this = fullfile(fd_save_base, 'DataVaryWinsize');
if ~exist(fd_save_this, 'dir'); mkdir(fd_save_this); end

for win_i=1:length(win_list)
  model_win = win_list(win_i);
  model_hop = hop_list(win_i);
  disp(model_win);
  ...
  fn_mat = fullfile(fd_save_this, sprintf('%s.%s.call_comp.modelData.winsize%d.mat', birdID, neuronID, model_win));
  save(fn_mat, 'd_all');
...
%% 3. Fit XGBoost models with different lags
...
metrics_all = cell(length(win_list), 1);
for win_i=1:length(win_list)
  model_win = win_list(win_i);
  fprintf('Fitting model for model_win %d...\n', model_win);
  % load data
  fn_d = fullfile(fd_save_this, sprintf('%s.%s.call_comp.modelData.winsize%d.mat', birdID, neuronID, model_win));
  a = load(fn_d);
  d_all = a.d_all;
  ...
  % only on the compound syllable
  d_input = d_comp;
  fd_save_res = fullfile(fd_save_this, 'res', sprintf('winsize%d', model_win));
  run_name = sprintf('winsize%d', model_win);
  % fit the model
  metrics = ZZfunc_runXGBoostPython_v1(d_input, pred_vae, output_name, group_name, n_fold_split, fd_save_res, fn_py, run_name);
  metrics_all{win_i} = metrics;
end



%% 4. Plot results
impv = cellfun(@(x) x.improvement_over_null, metrics_all, 'UniformOutput', false);
impv_array = cat(2, impv{:});
% impv_array = impv_array';

close all;
fig = ZZfunc_newFigurePDFsize_v1([10 10 600 600]);
ax = gca;
plot_mean_with_sem(ax, win_list, impv_array, [0.5 0.5 0.5], 1, '-', [0.5 0.5 0.5], 0.5);
xlabel(ax, 'Window size for averaging firing rate', 'FontSize', 14);
ylabel(ax, '% improvements over null', 'FontSize', 14);
