...
% differ from v1: loop through neurons
...
for di=1:size(d_list, 2)
  for mi=1:size(type_names, 2)
    ...
    fprintf('%d %s Building model for %s\n', ni, neuronID, run_name);
    d_input = d_list{di};
    pred_name = pred_list{mi};
    if size(d_input, 1)<200
      continue;
    end
...
%% Plot results
% close all; 
[fig, axs] = generatePanelGrid_v3(1, 3, [0.75], [], [0.075;0.05], [0.075;0.05], 0.075, [0], [10 10 1500 500], false);
