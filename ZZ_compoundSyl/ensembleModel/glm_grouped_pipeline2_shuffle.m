function [mdl, metrics, coefTbl, testPredTbl] = glm_grouped_pipeline2_shuffle(d_plot, pred_names, output_name, group_name, dist, link, train_test_ratio, offset_name)
...
%       d_plot, {'t_onset','vae1','vae2'}, 'spk', 'poisson','log', 0.8, 'sessionID', 'exposure');
    ...
    % permute the output variable of the training data
	  n = height(Ttr);  % number of rows
    temp = Ttr.(output_name);
    Ttr.(output_name) = temp(randperm(n));
