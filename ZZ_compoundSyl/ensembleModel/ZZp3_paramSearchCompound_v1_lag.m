...
% different lags between the firing rate window and VAE sliding window
...
% try different lags
lag_list = -25:5:50;
fd_save_this = fullfile(fd_save_base, 'DataVaryLag');
if ~exist(fd_save_this, 'dir'); mkdir(fd_save_this); end

for lag_i=1:length(lag_list)
  lag = lag_list(lag_i);
  disp(lag);
  ...
  fn_mat = fullfile(fd_save_this, sprintf('%s.%s.call_comp.modelData.lag%d.mat', birdID, neuronID, lag));
  save(fn_mat, 'd_all');
...
metrics_all = cell(length(lag_list), 1);
for lag_i=1:length(lag_list)
  lag = lag_list(lag_i);
  fprintf('Fitting model for lag %d...\n', lag);
  % load data
  fn_d = fullfile(fd_save_this, sprintf('%s.%s.call_comp.modelData.lag%d.mat', birdID, neuronID, lag));
  a = load(fn_d);
  d_all = a.d_all;
...
ax = gca;
plot_mean_with_sem(ax, lag_list, impv_array, [0.5 0.5 0.5], 1, '-', [0.5 0.5 0.5], 0.5);
xlabel(ax, 'Lags between firing and VAE windows', 'FontSize', 14);
