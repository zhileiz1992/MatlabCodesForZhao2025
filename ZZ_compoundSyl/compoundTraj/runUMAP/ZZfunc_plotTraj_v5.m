function [img, ax, x_lim, y_lim, x_all, y_all] = ZZfunc_plotTraj_v5(ax, embed, info, num_plot, batches, r_seed, col_dict, marg, col_alpha, end_alpha, x_lim, y_lim, syl_type, mark_end)
% given a embedding data table, plot trajectories of each syllable
% rasterized the scatter plot for easy edit in Illustrator
% differ from v4 in the call acoustic space folder: use the 'label' field instead of 'label' field
...
% sample syllables
if isempty(syl_type)
  syl_type = unique(info.label, 'sorted');
end
syl_select = {};
rng(r_seed);
for ii=1:size(syl_type,2)
  i_this = find((strcmp(info.label, syl_type{ii})) & (ismember(info.batch, batches)));
...
for i = 1:length(syl)
  c = col_dict.(syl{i});
  h_legend(i) = plot(ax, nan, nan, 'o', 'MarkerSize', 10, 'Color', c, 'MarkerFaceColor', c, 'DisplayName', syl{i});
end
hold(ax, 'off');
legend(ax, h_legend, 'Location', 'southeast', 'FontSize', 12);
end
