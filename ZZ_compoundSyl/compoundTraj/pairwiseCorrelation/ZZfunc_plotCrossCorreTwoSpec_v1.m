function [fig, axes_all, distm, t1, t2] = ZZfunc_plotCrossCorreTwoSpec_v1(comp, idx_rd)
% idx_rd = randsample(1:size(comp,1), 2);

% read wav then plot spectrogram
fs = 20000;
pad_pre = 0.032; pad_pre_pt = floor(fs*pad_pre);  % pad equal amount as in VAE sliding
pad_post = 0; pad_post_pt = floor(fs*pad_post);
sounds = cell(2, 1);
