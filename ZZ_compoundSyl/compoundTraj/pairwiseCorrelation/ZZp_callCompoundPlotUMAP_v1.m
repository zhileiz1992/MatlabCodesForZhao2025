...
fd_umap = fullfile(fd_save_ref, 'umap', sprintf('v%d', vi));
...
len_dcall = cellfun(@(x) size(x,1), calls_rd.dvae);
len_dcomp = cellfun(@(x) size(x,1), comp_this.dvae);
...
% plot trajectory of one call
  ...
  onset_this = floor((size(u1,1)-win_size)/mean_dur * onset + win_size/2); 
  offset_this = floor((size(u1,1)-win_size)/mean_dur * offset + win_size/2); 
...
ax = ZZfunc_addSimpleLegend_v2(ax, {'Call other', 'Call prim.', 'Comp. other', 'Comp. prim'}, {col_call_bg, col_call_prim, col_comp_bg, col_comp_prim}, 14, [12 12 12 12], linspace(4.5,2.5,4));
...
y_lim = [2 16];
ax = ZZfunc_rasterizeScatter_v1(ax, x_lim, y_lim);
ax = ZZfunc_addSimpleLegend_v2(ax, {'Call other', 'Call prim.'}, {col_call_bg, col_call_prim}, 14, [12 12], linspace(4.5,3.5,2));
xlabel(ax, 'UMAP axis 1', 'FontSize', 12);
ylabel(ax, 'UMAP axis 2', 'FontSize', 12);
...
%   scatter(ax, u2(other_idx,1), u2(other_idx,2), 10, 'filled', 'MarkerFaceColor', col_comp(ri,:), 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 1);
  scatter(ax, u2(inside_idx,1), u2(inside_idx,2), 30, 'filled', 'MarkerFaceColor', col_comp(rii,:), 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 1);
