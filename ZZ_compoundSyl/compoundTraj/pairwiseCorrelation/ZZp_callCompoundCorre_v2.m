...
% Zhilei, 09/23/2025
% differ from v1: stricter criteria for identifying high-similarity strands; remove the first 16 windows which is mostly
% silence
...
% remove the first and last half-sliding window
half_win = floor(win_size/2);
d1c = d1((half_win+1):(size(d1,1)-half_win), :);
d2s = comp.dvae; 
...
%% 4. Plot example cross-distance matrix
...
spec_chop = spec(freq_i, (half_win+1):(size(spec,2)-half_win));
...
for ii=1:50
  ...
  close all;
  [fig, axes_all, distm] = ZZfunc_plotCrossCorreTwoSpec_v4(d1c, d2, spec_chop, sound2, [12 23], custom_map, gray, [1000 5000], [0 0.35]);
...
ref_l = (1:length(x_prob)) - half_win; 
...
spec_plot = spec(freq_i,(half_win+1):(size(spec,2)-half_win));
...
% find peaks 
param.gapSize = 10;  
param.thresholdFlatness = 0.015;  % first threshold to identify peak, 0.02 for v1
param.extendThreshold = 0.01; % after thresholding extend to lower threshold to certain percentage of the peak height
