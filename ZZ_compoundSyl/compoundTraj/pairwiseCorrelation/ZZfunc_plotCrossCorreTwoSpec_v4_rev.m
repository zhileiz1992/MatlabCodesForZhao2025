function [fig, axes_all, distm] = ZZfunc_plotCrossCorreTwoSpec_v4_rev(d1, d2, spec1, sound2, clim2, cmap1, cmap2, flim, clim_distm)
% differ from v43: reference syllable on the left panel
...
bottom_row_height = 0.16;
right_col_width = bottom_row_height * (size(d2,1) / size(d1,1));
...
[ax1, ~, ~,  ~, ~,  t1] = showAudioSpectrogramZZ_flexible_v1(sound2, fs, axes_all(1), flim, clim2, 256, 256, 236);
% [power, ~, ~, ~, f1, t1]=getAudioSpectrogramZZ_flexible_v1(sound2, fs, 256, 256, 236, flim, clim2);
% t1 = (t1 - pad_sound) * 1000;
axis(axes_all(1), 'off');

% freq_i = find((f>=flim(1)) & (f<=flim(2)));
imagesc(axes_all(2), spec1', [0.15 max(spec1(:))]); 
set(axes_all(2), 'YDir', 'normal'); set(axes_all(2), 'XDir', 'reverse'); 
...
set(axes_all(3), 'YDir', 'normal'); 
% set(axes_all(3), 'XDir', 'reverse'); 
axis(axes_all(3), 'off');
end
