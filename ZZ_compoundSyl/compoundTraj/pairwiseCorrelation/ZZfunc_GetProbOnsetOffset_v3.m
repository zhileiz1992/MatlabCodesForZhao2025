function [onsets, offsets] = ZZfunc_GetProbOnsetOffset_v3(prob_amp, dt, param)
% Identify the onsets and offset of peaks in the flatness curve
% differ from v2: do some smoothing; also allow two peaks to be adjacent
prob_amp = smoothdata(prob_amp, 'gaussian', param.smooth_w);

% close all; figure; plot(prob_amp);
...
% consider as boundary if 1) values smaller then the extended threshold; OR 2) begin to increase/decrease
...
if ~isempty(idxs)
  ...
  for ii=1:length(idxs)
    ...
    % extend to the left
    left_i = pidx-1;
    while (left_i>1) && (prob_amp(left_i)>extThre) && (prob_amp(left_i-1)<prob_amp(left_i))
      left_i = left_i-1;
    end
    % extend to the right
    right_i = pidx+1;
    while (right_i<(length(prob_amp)-1)) && (prob_amp(right_i)>extThre) && (prob_amp(right_i+1)<prob_amp(right_i))
      right_i = right_i+1;
    end
