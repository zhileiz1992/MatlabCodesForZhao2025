function [onsets, offsets] = ZZfunc_GetProbOnsetOffset_v1(prob_amp, dt, param)
...
if ~isempty(idxs)
  ...
  for ii=1:length(idxs)
    ...
    % extend to the left
    left_i = pidx-1;
    while (left_i>0) && (prob_amp(left_i)>(prob_amp(pidx)*per))
      left_i = left_i-1;
    end
    % extend to the right
    right_i = pidx+1;
    while (right_i<length(prob_amp)) && (prob_amp(right_i)>(prob_amp(pidx)*per))
      right_i = right_i+1;
    end
