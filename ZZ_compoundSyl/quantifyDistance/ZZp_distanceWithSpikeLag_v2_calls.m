...
% Zhilei, 10/03/2025
% differ from v2: do it for the calls
...
fd_save = fullfile(fd_base, 'Figures', pairID, 'CompoundSyl', birdID, 'embed', [vae_run '_distanceCalls']);
...
% only look at calls
spike_this = spike_this(strcmp(spike_this.category, 'v'),:);
% spike_this = spike_this(strcmp(spike_this.label, 'V'),:);
...
imagesc(ax1, lags, 1:size(info_neu,1), dist_mean_all);
% colormap(ax1, 'p');
