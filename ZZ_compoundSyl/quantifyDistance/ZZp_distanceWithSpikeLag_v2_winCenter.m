...
% Zhilei, 10/03/2025
% differ from v2: use the center of sliding window as reference
...
fd_save = fullfile(fd_base, 'Figures', pairID, 'CompoundSyl', birdID, 'embed', [vae_run '_distanceWinCenter']);
...
for bi=1:size(birdIDs,2)
  fd_res = fullfile(fd_base, 'Figures', pairIDs{bi}, 'CompoundSyl', birdIDs{bi}, 'embed', [vae_run '_distanceWinCenter']);
...
[fig, axs] = generatePanelGrid_v2(2, 1, [0.5 0.35], [0.05], [0.025;0.075], [0.15;0.05], 0.05, [0;0], [10 10 500 800]);
ax1 = axs(1); hold(ax1, 'on');
imagesc(ax1, lags, 1:size(dist_mean_all,1), dist_mean_all, [0.75 1]);
% colormap(ax1, 'p');
xlim(ax1, [lags(1) lags(end)]);
...
fd_save = fullfile(fd_base, 'Figures', pairIDs{1}, 'CompoundSyl', birdIDs{1}, 'embed', [vae_run '_distanceWinCenter']);
fn_fig = fullfile(fd_save, 'AllBirds.comp_dist.%s.pdf');
print(fig, fn_fig, '-dpdf', '-painters');
