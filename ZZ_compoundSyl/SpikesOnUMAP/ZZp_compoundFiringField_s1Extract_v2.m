...
% this step has been done for call syllables that are good clusters, but not for v0 (not clustered) and v (filtered before
% running clustering due to duration). Need to extract those here, and call them simply 'V'
...
%% 2. Extract ephys data for annotated syllable (voltage trace & sorted spike): for non-call syllables
...
%% 3. Extract ephys data for annotated syllable (voltage trace & sorted spike): for calls that are not clustered
% this may take a few minutes, save results to disk for later use
% how many seconds to extract before and after the annotated syllable
pad_pre = 0.1;
pad_post = 0.1;
...
% save the results into disk for late use
fn_save_seg = fullfile(fd_save, sprintf('%s.V.segments_all.replaced2.mat', birdID));
save(fn_save_seg, 'seg_selected', '-v7.3');
