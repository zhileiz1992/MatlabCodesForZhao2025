...
%% 2. Create a background image to show the overall aoucsitc space
% randomly select n number of syllables
num_plot = 1000;
rng(1992);
i_rd = randsample(1:size(info,1), num_plot);
...
for ii=1:length(i_rd)
  ...
  scatter(ax, u(:,1), u(:,2), 10, 'filled', 'MarkerFaceColor', '#969696', 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.01);
end
xlim(ax, x_lim);
ylim(ax, y_lim);
frame = getframe(ax); img = frame.cdata;
% save the image data for re-use
fn_img = fullfile(fd_field, sprintf('%s.background.img.mat', birdID));
save(fn_img, 'img');



%% 3.1 Plot firing fields: no filtering
fd_save_plot1 = fullfile(fd_field, 'plot_all');
if ~exist(fd_save_plot1, 'dir'); mkdir(fd_save_plot1); end
% distinguish different syllable catorgories
cat_plot = {'v', 'b', 'x', 'h', 'e'};
cat_color = {'#e41a1c', '#377eb8', '#a6761d', '#984ea3', '#4daf4a'};
...
for ni=1:size(info_neu,1)
  ...
  close all;
  [fig, axes] =  generatePanelGrid_v2(1, size(cat_plot,2), [0.5], [], [0.1;0.05], [0.05;0.05], 0.02, [0], [10 10 2000 700]);
  for si=1:size(cat_plot,2)
    cate = cat_plot{si};
    spike = spike_this(strcmp(spike_this.category, cate),:);
    % first plot the background
    ax = axes(si); cla(ax);hold(ax, 'on');
    xlim(ax, x_lim);
    ylim(ax, y_lim);
    % add the scatter background
    image(ax, ax.XLim, ax.YLim, flipud(img));
    % loop through syllables, plot where the spikes are
    for ri=1:size(spike,1)
      mat_loc = spike.mat_loc{ri};
      if ~isempty(mat_loc)
        syl_ID = spike.syl_ID{ri};
        i_inU = find(strcmp(info.syl_ID, syl_ID));
        ustart = info.ustart(i_inU);
        uend = info.uend(i_inU);
        u = umap_res(ustart:uend,:);
        %       scatter(ax, u(:,1), u(:,2), 10, 'filled', 'MarkerFaceColor', '#a6761d', 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.25);
        % plot where the spikes are
        scatter(ax, u(mat_loc,1), u(mat_loc,2), 30, 'filled', 'MarkerFaceColor', cat_color{si}, 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.2);
      end
    end
    xlim(ax, x_lim);
    ylim(ax, y_lim);
  ...
  % save results
  fn_fig = fullfile(fd_save_plot1, sprintf('%s.%s.firingFields.pdf', birdID, neuronID));
...
for ni=1:size(info_neu,1)
  ...
  [fig, axes] =  generatePanelGrid_v2(2, 3, [0.38;0.38], [0.08], [0.1;0.05], [0.05;0.05], 0.05, [0;0], [10 50 1200 900]);
  [fig2, axes2] =  generatePanelGrid_v2(2, 3, [0.38;0.38], [0.08], [0.1;0.05], [0.05;0.05], 0.05, [0;0], [10 50 1200 900]);
  for si=1:size(cat_plot,2)
    cate = cat_plot{si};
    spike = spike_this(strcmp(spike_this.category, cate),:);
  ...
  image(ax, ax.XLim, ax.YLim, flipud(img));
  % overlay spikes
  [ax, spike_loc] = ZZfunc_plotSpikeOnUmapComp_v2(ax, spike, umap_res, 30, '#377eb8', 0.2);
  xlim(ax, x_lim); ylim(ax, y_lim);
...
%   powerBlank = 
  ...
  for di=1:size(to_plot_d, 2)
    ...
    dbase_list = cell(size(fns_dbase, 1));
    ...
    sound_spike = [];
    act_num = min([150 size(d_plot,1)]);
    i_rd = 1:act_num; 
    parfor ii=1:length(i_rd)
      idx = i_rd(ii);
  ...
  for fig_i=1:num_fig
    close all;
    [fig, axs] =  generatePanelGrid_v2(3, 3, [0.2;0.03;0.6], [0.02;0.02], [0.05;0.05], [0.05;0.01], 0.01, [0;0;0], [10 50 1800 900]);
    for ii=1:3
      idx = (fig_i-1) * 3 + ii;
      % plot spectrogram
