...
%% 3. Plot spikes directly
x_lim = [0.5 19]; % the same x/y limits
y_lim = [-6.5 18.5];
close all;
% one plot for calls, another for compound syllables
[fig, axes] =  generatePanelGrid_v3(1, 2, [0.75], [], [0.1;0.05], [0.1;0.05], 0.05, [0], [10 10 1100 600], true);
ax1 = axes(1); cla(ax1); hold(ax1, 'on');
ax2 = axes(2); cla(ax2); hold(ax2, 'on');
% plot the backgrounds first
xlim(ax1, x_lim); ylim(ax1, y_lim);
img = img_all.v;
image(ax1, ax1.XLim, ax1.YLim, flipud(img));
xlim(ax2, x_lim); ylim(ax2, y_lim);
img = img_all.comp;
image(ax2, ax2.XLim, ax2.YLim, flipud(img));

% then plot spikes from neurons
% neuron_color = lines(size(crit,1));
neuron_color = {'#e41a1c','#a65628','#4daf4a','#984ea3','#ff7f00','#f781bf','#377eb8','#737373'};
for ni=1:size(neuron_sel,1)
  ...
  [ax1, spike_loc1] = ZZfunc_plotSpikeOnUmapComp_v2(ax1, spike1, umap_res, 30, neuron_color{ni}, 0.2);
  xlim(ax1, x_lim); ylim(ax1, y_lim);
  
  % plot for compound syllables
  spike2 = spike_this(ismember(spike_this.category, {'b', 'x'}),:);
  spike2 = spike2(spike2.dur>=0.3, :);
  [ax2, spike_loc2] = ZZfunc_plotSpikeOnUmapComp_v2(ax2, spike2, umap_res, 30, neuron_color{ni}, 0.2);
  xlim(ax2, x_lim); ylim(ax2, y_lim);
end
% add text
hold(ax1,'on');
[ax1] = ZZfunc_addSimpleLegend_v2(ax1, neuron_sel.Var1', neuron_color, 10, zeros(size(neuron_sel,1),1)+1.5, linspace(10,17,size(neuron_sel,1)));

fn_fig = fullfile(fd_save, 'manually_selected8.pdf');
