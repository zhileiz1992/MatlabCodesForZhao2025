...
% differ from v2: try merge spike and density in one plot
...
% create a univeral embedding of all syllable types
close all;
fig_size = [10 10 600 600];
fig = ZZfunc_newFigurePDFsize_v1(fig_size); 
ax = gca; cla(ax);
% first all other syllable types
ax = ZZfunc_plotEmbedGray_v1(ax, info, umap_res, i_comb, x_lim, y_lim, 10, '#737373', 0.2);
...
% the background without blue regions
a=load(fullfile(fd_field, sprintf('%s.background.img.mat', birdID)));
...
fd_save_both = fullfile(fd_save_plot2, 'both_overlay_density');
if ~exist(fd_save_both, 'dir'); mkdir(fd_save_both); end
...
for ni=1:size(info_neu,1)
  ...
  close all;
  % the same plot for both spikes and density
  ...
  for si=1:size(cat_plot,2)
    ...
    % first plot the acoustic space
    axloc = cat_loc{si};
    ax = axes(axloc(1), axloc(2)); cla(ax); hold(ax, 'on');
    xlim(ax, x_lim); ylim(ax, y_lim);
    % add the scatter background
%     img = img_all.(cate);
    h1=image(ax, ax.XLim, ax.YLim, flipud(img));
    set(h1, 'AlphaData', 0.5);
    
    % overlay spikes
    [ax, spike_loc] = ZZfunc_plotSpikeOnUmapComp_v2(ax, spike, umap_res, 10, 'black', 0.5);
    xlim(ax, x_lim); ylim(ax, y_lim);
    
    % then calculate and plot density 
%     spike_loc = ZZfunc_plotSpikeOnUmapComp_v2_noplot(ax, spike, umap_res);
...
%       d_qt = quantile(density(:), 0.999);
     d_qt = max(density(:)) * 1.5;
      d_qt = max([1 d_qt]);
      h2=imagesc(ax, x_centers, y_centers, density, [0, d_qt]);
...
%     d_qt = quantile(density(:), 0.999);
  ...
  % save results
  fn_fig = fullfile(fd_save_both, sprintf('%s.%s.both_spike_density.pdf', birdID, neuronID));
