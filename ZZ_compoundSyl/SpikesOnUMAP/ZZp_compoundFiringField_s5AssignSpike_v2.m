...
% differ from v1: add dbase and fn_wav information to the spike_embed struct, so later can load sound file
clear; close all;
...
for ri=1:size(seg_selected,2)
  ...
  spike_embed(ri).title_str = seg.title_str;
  spike_embed(ri).fn_audio = seg.fn_audio;
  spike_embed(ri).fn_dbase = seg.fn_dbase;
  spike_embed(ri).ch = seg.ch;
  spike_embed(ri).seg_start_ori = seg.seg_start_ori;
  spike_embed(ri).seg_end_ori = seg.seg_end_ori;
  spike_embed(ri).seg_start = seg.seg_start;
  spike_embed(ri).seg_end = seg.seg_end;
  spike_embed(ri).dur = (seg.seg_end_ori - seg.seg_start_ori) /fs;
  % find where the spikes are
  spike_i = find(seg.spike_iv);
  spike_embed(ri).spike_i = spike_i;
  if ~isempty(spike_i)
