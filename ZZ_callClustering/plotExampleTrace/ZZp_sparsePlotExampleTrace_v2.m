...
% Zhilei, 09/14/2025
% differ from v1: get high-res spectrogram image
...
bis = [1, 1, 2, 2, 4, 3];
neuronIDs = {'20240903-ch6', '20240917-ch12A', '20240831-ch9', '20240919-ch12', '20240225-ch7', '20240903-ch9'};
fis = [104, 153, 43, 60, 152, 535];
ts = [1.2, 3, 3.6, 2, 0.5, 1.1];
clims = {[12 22.5]; [12 22.5]; [12 25]; [12 25]; [12 25]; [12 25]};
...
%   ch_num = str2num(strrep(ch, 'ch', ''));
  ...
  % plot
  close all; 
  [fig, axes] = generatePanelGrid_v2(2, 1, [0.4;0.4], [0.05], [0.1;0.05], [0.05;0.05], 0.05, [0 0], [10 10 1200 200]);
  set(fig, 'Renderer', 'painters');
  % first spectrogram
  ax1 = axes(1);
  [power1, powerRaw, powerGrey, S, f, t] = getAudioSpectrogramZZ_flexible_v1(aud_s, fs, 512, 512, 492, [250 6500], clim);
...
%   exportgraphics(fig, fn_fig, 'Resolution', 600);
  ...
  uiopen(fn_fig,1)
  f = gcf();
%   im = findAllMatchingChildren(f, @(g)isa(g, 'matlab.graphics.primitive.Image'));
  im = findall(f, '-isa', 'matlab.graphics.primitive.Image');
  fn_png = fullfile(fd_save_this, sprintf('%s.%s.t%.3f.tiff', birdID, n, ts(ni)));
  imwrite(flipud(im.CData), fn_png);
