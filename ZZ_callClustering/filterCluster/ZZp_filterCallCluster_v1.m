...
thres = [0.12 0.12 0.09 0.12];
clims = {[10 21]; [10 21]; [10 21]; [15 25]};  % color limits when calculating spectrograms
...
%% 2. Calculate the standard deviation of spectrogram features
...
for cid = 1:max(embed.hdbscan_cluster)
  ...
  t_all{cid} = t;
  stacked_all{cid} = stacked_array;
end

% calculate sparseness
sparse_all = {}; % calculate a sparseness index along the freq. axis
for cid = 1:max(embed.hdbscan_cluster)
  % calculate std and CV at each moment of the call
  % focus on certain freq range
  f_range = [0 10000]; 
  f_idx = find((f>=f_range(1)) & (f<=f_range(2)));
  stacked_array = stacked_all{cid};
