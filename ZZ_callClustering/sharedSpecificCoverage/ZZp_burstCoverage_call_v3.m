...
% Zhilei, 07/11/2025
% differ from v3: excluding comparison where there are too much missing data
...
for ni=1:size(all_neuron, 2)
  ...
  if (size(seg_this1, 2)>=min_trial) && (size(seg_this2, 2)>=min_trial)
    ...
    % check if the neuron passed
    % if in both plots and pass the sparseness threshold, then pass both
    sparse_thre = 0.25; 
    if cn1.isPass && spa1>= sparse_thre && cn2.isPass && spa2>= sparse_thre
      t_all(ni).isPass1 = 1;
      t_all(ni).isPass2 = 1;
    end 
    % if in one plot but not the other, relax the criteria, i.e. make specific more conservative
    prop_thre = 0;  % include neuron if more than this proportion of renditions have spikes around the peak PSTH, set 0.25
    psth_thre = 10; 
    ircc_thre = 0.1; 
    min_rend = 3;  % minimal number of renditions that have spikes around the peak PSTH
    if cn1.isPass && spa1>= sparse_thre && (~cn2.isPass)
      t_all(ni).isPass1 = 1;
      t_all(ni).isPass2 = (cn2.num_rend_psth_fire>=min_rend) & (cn2.peak_psth>=psth_thre)...
        & (cn2.ircc>=ircc_thre) & (spa2>=sparse_thre);
    end
    if cn2.isPass && spa2>= sparse_thre && (~cn1.isPass)
      t_all(ni).isPass1 = (cn1.num_rend_psth_fire>=min_rend) & (cn1.peak_psth>=psth_thre)...
        & (cn1.ircc>=ircc_thre) & (spa1>=sparse_thre);
      t_all(ni).isPass2 = 1;
    end
...
% for shared, plot where the bursts are, connected by a small line
for ii=1:length(t_shared1)
  plot([t_shared1(ii) t_shared2(ii)], [1.5 2], '-o', 'MarkerSize', 10, 'MarkerFaceColor', 'b', 'MarkerEdgeColor', 'none',...
    'Color', 'b', 'LineWidth', 1);
  hold on;
end
% for specific, just plot dots
plot(t_specific, zeros(size(t_specific))+1, 'o',  'MarkerSize', 10, 'MarkerFaceColor', 'r', 'MarkerEdgeColor', 'none');
ylim([0.5 2.5]);
yticks([1 1.75]);
yticklabels({'Specific', 'Shared'});
ax = gca;
ax.YAxis.FontSize = 12;
temp = [t_shared1 t_shared2 t_specific];
