...
% differ from v6: don't color the clusters that doesn't pass the filtering, swap the call subtype names; change colors as well
...
to_removes = {{'v6'}; {'v5', 'v6'}; {'v1','v5','v6','v7'}; {'v3','v4','v5'}};  % what call subtypes to remove (oldID)
...
% % bird M2
% oldID = {'v1', 'v4', 'v3', 'v2', 'v5', 'v6'};
% newID = {'v1', 'v2', 'v3', 'v4', 'v5', 'v6'};
% % bird M3
oldID = {'v2', 'v3', 'v4', 'v1', 'v6', 'v7', 'v5'};
newID = {'v1', 'v2', 'v3', 'v4', 'v5', 'v6', 'v7'};
% bird M4
...
% col_filtered = {'#969696','#737373','#525252','#252525'};  % for filtered clusters, use different shades of grey
...
bi = 3;
bd = birdIDs{bi};
to_remove = to_removes{bi};
% where to save results
fd_save = fullfile(fd_save_base, bd, fd_vae, syl, vae_run, 'plotsFiltered');
...
for di=1:size(ds,1)
  ...
  scatter(x(mask_noise), y(mask_noise), 6, 'filled', 'MarkerFaceColor', [0.85 0.85 0.85], 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.25, 'MarkerEdgeAlpha', alpha);
  % Plot other clusters
...
for cid = 1:max(embed.hdbscan_cluster)
  ...
  parfor ii=1:size(segs,2)
    ...
    [power, powerRaw, powerGrey, S, f, t] = getAudioSpectrogramZZ_flexible_v1(seg_aligned, fs, 256, 256, 236, [250 7500], clims{bi});
...
%% plot mean spectrogram with different scaling
close all;
...
% save to pdf
fn_pdf = fullfile(fd_save, sprintf('%s.spect.MeanWrap2.pdf', bd));
print(fig, fn_pdf, '-dpdf', '-painters');
...
%% pad excluding those that are not in final use
