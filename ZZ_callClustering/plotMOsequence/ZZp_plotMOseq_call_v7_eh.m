...
% plot PSTH and sequences for click and squawk
...
% grab the extracted data struct for clicks and squawks
fd_data = fullfile(fd_home, 'Figures', pairID, 'CompoundSyl', birdID, 'extractedReplaced2');
syls = {'e', 'h'}; 
% where to save results
fd_save = fullfile(fd_save_master, birdID, 'popRaster_eh');
...
for li=1:size(syls,2)
  % load previous saved data struct
  syl_label = syls{li};
  fprintf('Plotting for %s %s...\n', birdID, syl_label);
  a = load(fullfile(fd_data, sprintf('%s.%s.segments_all.replaced2.mat', birdID, syl_label)));
  seg_selected = a.seg_selected;
  pt = 'chan0';  % focus on production
  seg_selected = seg_selected(strcmp({seg_selected.aud_ch}, pt)); 
  clear a; 
  
    % perform filtering on the sound
  for si=1:size(seg_selected, 2)
     audio_filtered = ZZ_butterFilterFunc_v1(seg_selected(si).signalRaw, fs, 500, 10000, 2);
     seg_selected(si).signalFiltered = audio_filtered;
  end
