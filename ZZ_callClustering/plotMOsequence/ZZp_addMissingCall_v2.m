...
% differ from v1: take the latest neuron_ordered data; focus on V2 from bird M1
...
% crossM = {{{'v4', 'v5'}, {'v1', 'v7'}}, ...
%           {{'v1', 'v4'}}};
% crossM = {{{'v2', 'v4'}, {'v2', 'v5'}, {'v2', 'v1'}, {'v2', 'v7'}}};
crossM = {{}, {}, {{'v2', 'v3'}, {'v1', 'v2'}, {'v1', 'v3'}}, {}};
...
for ci=1:size(cM, 2)
  ...
  for flip_i=1:2
    ...
    fn_order = fullfile(fd_hahn, sprintf('Hahnloser-%s-chan0.neuron_orderedPlotted5.mat', ref_v));
    load(fn_order);
    fn_ren = fullfile(fd_hahn, sprintf('Hahnloser-%s-chan0.sampled_rends_ordered5.mat', ref_v));
    load(fn_ren);
    % load extracted data struct for target syllable
    fd_d = fullfile(fd_home, 'Figures', pairID, 'HahnloserNew', birdID, 'extractedPull');
    fn_d = sprintf('%s.%s.segments_all.pull.mat', birdID, syl_v);
...
fn_info = fullfile(fd_info, sprintf('%s_addMissingCall3.txt', birdID));
writetable(T, fn_info, 'Delimiter', '\t', 'WriteVariableNames', false);
  

% load the info regarding missing neuron-call subtype pairs
fn_info = fullfile(fd_home, 'DbaseFiles', pairID, 'MetaInfo', sprintf('%s_addMissingCall3.txt', birdID));
