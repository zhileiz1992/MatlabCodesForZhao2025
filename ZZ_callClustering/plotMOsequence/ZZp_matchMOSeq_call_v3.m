...
% differ from v2: add plots with same absolute time scale
...
for vi=1:size(fds,1)
  ...
  fn_add = fullfile(fd_save_master, birdID, 'extractedAdded', sprintf('%s.%s.segments_all.added.mat', birdID, syl_this));
  ...
  fn_manual = fullfile(fd_save_master, birdID, 'extractedAdded', sprintf('%s.%s.segments_all.manual.mat', birdID, syl_this));
  ...
  for ri=1:size(syls,2)
    ...
    fd_save = fullfile(fd_save_master, birdID, 'callMatch2', sprintf('ref_%s', ref_v));
