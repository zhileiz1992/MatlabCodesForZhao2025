...
% differ from v2: rename the call subtypes output by HDBSCAN
...
embed2 = embed; 
% replace the call subtype names here
for ri=1:size(embed2,1)
  v_old = embed2.hdbscan_cluster(ri);
  if v_old>0
    i_old = find(strcmp(no, sprintf('v%d', v_old)));
    v_new = str2num(strrep(nn{i_old}, 'v', ''));
    embed2.hdbscan_cluster(ri) = v_new;
  end
end
% save for later use
fn_embed2 = fullfile(fd_d, sprintf('%s.%s.embeddingReplaced.csv', bd, vae_run)); 
writetable(embed2, fn_embed2);
embed = embed2;
