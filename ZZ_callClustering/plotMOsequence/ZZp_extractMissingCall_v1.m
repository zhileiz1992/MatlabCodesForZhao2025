...
% fn_info = fullfile(fd_home, 'DbaseFiles', pairID, 'MetaInfo', sprintf('%s_addMissingCallComb.txt', birdID));
...
for ci=1:size(unique_type, 1)
  ...
  % get unique neuronID to avoid extract more than once
  neuron_unique = unique(info.Var1(idx), 'stable');
  segments_all = {};
  parfor rii=1:size(neuron_unique, 1)
    segments_all{rii} = [];
    % get date and channel id
    neuronID = neuron_unique{rii};
  ...
  fn_save_seg = fullfile(fd_save_this, sprintf('%s.%s.segments_all.added.mat', birdID, syl));
