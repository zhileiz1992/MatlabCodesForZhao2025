...
% differ from v5: use the replaced names for call subtypes; use stricter criteria for selecting sparse neurons for calls
...
fds = dir(fullfile(fd_save_master, birdID, 'extractedReplaced', '*segments_all.replaced.mat'));
% where to save results
fd_save = fullfile(fd_save_master, birdID, 'popRaster');
...
%       fn_pdf = '';
  ...
  fn_criteria = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.criteria6.mat', syl_label, pt));
  save(fn_criteria, 'criteria');
  fn_neu_order = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.neuron_ordered6.mat', syl_label, pt));
...
%   disp(setdiff(nsold, neuron_ordered));
  ...
  fn_plot = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.tDiff6.fig', syl_label, pt));
  ...
  fn_order_new = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.neuron_orderedPlotted6.mat', syl_label, pt));
  save(fn_order_new, 'neuron_ordered');
  fn_sampled_rends = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.sampled_rends_ordered6.mat', syl_label, pt));
  save(fn_sampled_rends, 'sampled_rends_ordered');
  
  % plot with the same absolute time scale
  close all;
  fn_plot = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.tSame6.fig', syl_label, pt));
  r_seed = 1992;
...
%   tlims = [-0.1; 0.5];  % or use the range of other syllable types
  ...
  fn_t = fullfile(fd_save_this, sprintf('Hahnloser-%s-%s.tSame.xlims6.mat', syl_label, pt));
  save(fn_t, 'xlims');
