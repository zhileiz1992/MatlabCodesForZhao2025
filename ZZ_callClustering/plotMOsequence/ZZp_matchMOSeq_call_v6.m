...
% differ from v4: use the latest/stricter selection of sparse neurons
...
fds = dir(fullfile(fd_save_master, birdID, 'extractedReplaced', '*.v*.segments_all.replaced.mat'));
...
for vi=1:size(fds,1)
  ...
  for ri=1:size(syls,2)
    ...
    fd_save = fullfile(fd_save_master, birdID, 'callMatch6', sprintf('ref_%s', ref_v));
    if ~exist(fd_save, 'dir')
      mkdir(fd_save);
    end
    % load the neuron order and renditions plotted
    fn_order = fullfile(fd_save_master, birdID, 'popRaster', ref_v, sprintf('Hahnloser-%s-chan0.neuron_orderedPlotted6.mat', ref_v));
    load(fn_order);
    fn_ren = fullfile(fd_save_master, birdID, 'popRaster', ref_v, sprintf('Hahnloser-%s-chan0.sampled_rends_ordered6.mat', ref_v));
    load(fn_ren);
