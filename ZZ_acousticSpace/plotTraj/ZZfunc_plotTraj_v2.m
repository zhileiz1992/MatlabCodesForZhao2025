function [img, ax] = ZZfunc_plotTraj_v2(ax, embed, col_dict, marg, col_alpha, end_alpha, x_lim, y_lim)
% given a embedding data table, plot trajectories of each syllable
% rasterized the scatter plot for easy edit in Illustrator
% differ from v1: add option to set x_lim and y_lim directly. 

% determine xy limits if not set
if isempty(x_lim); x_lim = [min(embed.umap1)-marg, max(embed.umap1)+marg]; end
if isempty(y_lim); y_lim = [min(embed.umap2)-marg, max(embed.umap2)+marg]; end
