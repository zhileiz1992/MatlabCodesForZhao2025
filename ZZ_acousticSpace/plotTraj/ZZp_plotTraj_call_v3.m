...
% Zhilei, 08/05/2025
% differ from v2: use the UMAP results that are trained on all syllable renditions
...
fd_umap = fullfile(fd_data, birdID, 'Traj', 'applySylAll', umap_run);
fn_umap = sprintf('%s.%s.embedding.csv', birdID, umap_run);
embed = readmatrix(fullfile(fd_umap, fn_umap));
fn_info = sprintf('%s.%s.info.csv', birdID, umap_run);
info = readtable(fullfile(fd_umap, fn_info), 'Delimiter', ',');
...
% create the scatter plot, rasterized it to img, then replot with other element editable
close all;
...
marg = 1;
col_alpha = 0.075;  % transparency of points in trajectory, default 0.05
end_alpha = 0.2;  % transparency of start/end points, default 0.2
num_plot = 1000;
% num_plot = 10000;
batches = {'batch1'};
r_seed = 1992;
[img, ax] = ZZfunc_plotTraj_v3(ax, embed, info, num_plot, batches, r_seed, col_dict, marg, col_alpha, end_alpha, [], []);
% save figure
...
% umap_run = 'umapAll.v4v5';
% umap_run = 'umapAll.v1v7';
umap_run = sprintf('umapAll.%s', syl_str);
fd_umap = fullfile(fd_data, birdID, 'Traj', 'applySylAll', umap_run);
fn_umap = sprintf('%s.%s.embedding.csv', birdID, umap_run);
embed = readmatrix(fullfile(fd_umap, fn_umap));
fn_info = sprintf('%s.%s.info.csv', birdID, umap_run);
...
x_lim = [min(embed(:,1))-marg, max(embed(:,1))+marg];
y_lim = [min(embed(:,2))-marg, max(embed(:,2))+marg];
...
for si=1:size(syl, 2)
  ...
  fig = ZZfunc_newFigurePDFsize_v1(fig_size);
  ax = gca; 
  [img, ax] = ZZfunc_plotTraj_v4(ax, embed, info, num_plot, batches, r_seed, col_dict, marg, col_alpha, end_alpha, [], [], {v});
  % save figure
  fn_pdf = fullfile(fd_save, sprintf('%s.ind.%s.pdf', birdID, v));
  print(fig, fn_pdf, '-dpdf', '-painters');
end
