...
% Zhilei, 09/11/2025
% differ from v5: correct the time axis on neural SVM matrix
...
fd_save = fullfile(fd_base, 'Figures', pairID, 'AcousticSpace', birdID, 'embed', [vae_run '_decodePopMatrix5'], 'masked3');
...
for ii=1:size(syls,2)
  for jj=ii:size(syls,2)
    ...
    tendy = ty(end) - pad + bin + tpad_n(2);
    iy = find((ty>=-tpad_n(1)) & (ty<=tendy));
    tendx = tx(end) - pad + bin + tpad_n(2);
...
% also check how many neurons were used for population plot
num_neu = nan(size(syls,2), size(syls,2)); 
for ii=1:size(syls,2)
  for jj=ii:size(syls,2)
    % check neurons used for decoding 
    run_name = sprintf('%s_%s', syls{ii}, syls{jj});
    fn_pop = fullfile(fd_data, sprintf('%s.%s.%s.pop.mat', birdID, run_name, syls{jj}));
    if exist(fn_pop, 'file')
      a=load(fn_pop); num_neu(ii,jj)=size(a.pop2,2); 
    else
      num_neu(ii,jj)=nan; 
    end
...
clim_n = clims_n_all{bi};
ZZfunc_showMMDmatrix_v5(ax, neu3, clim_n, v_loc, syls, 'Neural SVM accuracy', true, line_loc, '#a65628', '--', 1, gray, false);
...
v_loc = cumsum(lens_n)-lens_n/2;
line_loc = cumsum(lens_n);
ZZfunc_showMMDmatrix_v5(ax, neu4, clim_n2, v_loc, syls, 'Neural similarity', true, line_loc, '#a65628', '--', 1, flipud(gray), false);
...
line_loc = cumsum(lens_n);
clim_n2 = clims_n2_all{bi};
ZZfunc_showMMDmatrix_v5(ax, neu4', clim_n2, v_loc, syls, 'Neural similarity', true, line_loc, '#a65628', '--', 1, flipud(gray), true);
...
clim_n2 = clims_n2_all{bi};
ZZfunc_showMMDmatrix_v5(ax, neu6', clim_n2, v_loc, syls, 'Neural similarity', true, line_loc, '#a65628', '--', 1, flipud(gray), true);
...
range_n = [0.5 1];  % what range to normalize to 
range_a = clims_a_all{bi};
clims_n2 = clims_n2_all{bi};
clims_a2 = clims_a2_all{bi};
% acoustic MMD
vmin = clims_a_all{bi}(1); 
vmax = clims_a_all{bi}(2);
for ii=1:size(syls,2)
...
p = {'v2', 'v1'}; % first syllable on the y-axis (height)
close all;
