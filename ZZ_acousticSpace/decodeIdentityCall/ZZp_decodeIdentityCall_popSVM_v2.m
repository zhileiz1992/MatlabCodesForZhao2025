...
% Zhilei 08/28/2025
% differ from v2: add a shuffled curve for MMD, make sure the chance level for neural and acoustic are aligned
...
% calculate relative time of the bin start
max_bin_num = max([size(pop1,3) size(pop2,3)]);
...
xlim2 = min([rel_t_mmd(end) rel_t(end)]);
yyaxis left
plot_mean_with_ci(ax, rel_t(1:min_bin), squeeze(accu_all(:,1,:))', col_list{1}, 3, '-', hexToRGB(col_list{1}), 0.5);
plot_mean_with_ci(ax, rel_t(1:min_bin), squeeze(accu_all(:,2,:))', faintColor_v1(hexToRGB(col_list{1}),0.5), 3, ':', faintColor_v1(hexToRGB(col_list{1}),0.5), 0.5);
