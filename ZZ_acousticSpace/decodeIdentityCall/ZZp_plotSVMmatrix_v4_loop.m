...
% Zhilei, 09/05/2025
% differ from v3: mask call subtypes that are not good clusters
...
fd_save = fullfile(fd_base, 'Figures', pairID, 'AcousticSpace', birdID, 'embed', [vae_run '_decodePopMatrix4'], 'masked');
...
for ii=1:size(syls,2)
  for jj=ii:size(syls,2)
    ...
    % perform masking 
    if (ismember(syls{ii}, to_mask)) || (ismember(syls{jj}, to_mask))
      temp(:)= 1; 
    end
...
for ii=1:size(syls,2)
  for jj=ii:size(syls,2)
    ...
    imagesc(ax, tx, ty, a_this, clim_a);
    ...
    imagesc(ax, tx, ty, n_this, clim_n);
    colormap(ax, 'gray'); colorbar(ax);
    set(ax, 'YDir', 'reverse');
    axis(ax, 'equal');
    axis(ax, 'tight');
