...
% differ from v4: use the renames where low-quality clusters are identified, exclude those from plot
...
% rename relationship
syls_all_list = {{'v1', 'v2', 'v3', 'v4', 'v5', 'v6', 'v7'}, ...
             {'v1', 'v2', 'v3', 'v4'}, ...
             {'v1', 'v2', 'v3', 'v4', 'v5'}, ...
             {'v1', 'v2', 'v3', 'v4'}};
...
fd_save = fullfile(fd_base, 'Figures', pairID, 'AcousticSpace', birdID, 'embed', [vae_run '_decodePopMatrix5'], 'intermediate');
...
%   load(fn_e); e_all{syl_i} = seg_selected(strcmp({seg_selected.aud_ch}, 'chan0'));
  ...
  fn_n = fullfile(fd_hahnloser, 'popRaster2', ss, sprintf('Hahnloser-%s-chan0.%s.mat', ss, suffix_hahn));
  load(fn_n); n_all{syl_i} = neuron_ordered;
  fn_c = fullfile(fd_hahnloser, 'popRaster2', ss, sprintf('Hahnloser-%s-chan0.%s.mat', ss, suffix_criteria));
...
% skip if two few common neurons
if size(neuron_sort,1)<5; continue; end  
