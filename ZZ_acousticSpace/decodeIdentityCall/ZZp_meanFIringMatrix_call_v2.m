...
% Zhilei 08/30/2025
% differ from v1: overlay the MMD curve on neural curve; also calculate off-diagonal differences
...
% add shuffle-label data as well
mean1 = nan(size(neuron_sort,1), size(bin_sums1,2));
mean2 = nan(size(neuron_sort,1), size(bin_sums2,2));
% min_bin = min([size(bin_sums1,2) size(bin_sums2,2)]);
% shuff1 = nan(size(neuron_sort,1), min_bin);
% shuff2 = nan(size(neuron_sort,1), min_bin);
% rng(1992);
...
%   % shuffle the call subtype label, but keep the number of trials unchanged
%   comb_sum = cat(1, bin_sums1(idx1,1:min_bin),  bin_sums2(idx2,1:min_bin));
%   ird = sample_subarrays(1:size(comb_sum,1), [length(idx1), length(idx2)]);
%   shuff1(ni,:) = mean(comb_sum(ird{1},:),1);
%   shuff2(ni,:) = mean(comb_sum(ird{2},:),1);
end
...
% shuffle_diff = abs(shuff1 - shuff2);
% mean_shuffle_diff = mean(shuffle_diff,1);
...
% acoustic_neural.mean_shuffle_diff = mean_shuffle_diff;
...
% overlay acoustic MMD curve with neural difference curve
close all;
...
yyaxis left
plot(ax, rel_t(1:length(mean_m_diff)), mean_m_diff, 'LineWidth', 3, 'Color', col_list{1}, 'LineStyle', '-');
ylim(ax, [0 max(mean_m_diff)*1.05]);
ylabel(ax, 'Mean firing rate difference', 'FontSize', 14);
ax = ZZfunc_addSimpleLegend_v2(ax, {'Neural', 'Acoustic'}, col_list, 16, [-0.02 -0.02], [0.95 0.92]);

yyaxis right
plot(ax, rel_t_mmd, diag_mmd, 'LineWidth', 3, 'Color', col_list{2}); 
plot(ax, rel_t_mmd, diag_shuff, 'LineWidth', 3, 'Color', faintColor_v1(hexToRGB(col_list{2}),0.5), 'LineStyle', ':'); 
xlim(ax, [xlim1 xlim2]);
ylim(ax, [mean(diag_shuff) max(diag_mmd)*1.05]);
...
fn_pdf = fullfile(fd_save, sprintf('%s.%s_%s.MMDvsMeanDiff_comb.pdf', birdID, syls{1}, syls{2}));
print(fig, fn_pdf, '-dpdf', '-painters');


%% 5. Perform cross-correlation
x = mean_m_diff;
t_x = rel_t(1:length(mean_m_diff)); 
y = diag_mmd;
t_y = rel_t_mmd;
...
%% 6. Calculate off-diagonal distance
dist_all = nan(size(mean1,2), size(mean2,2));
for tii=1:size(mean1,2)
  for tjj=1:size(mean2,2)
    m1 = mean1(:, tii);
    m2 = mean2(:, tjj);
    dist_all(tii,tjj) = mean(abs(m1-m2));
  end
end
...
fig = ZZfunc_newFigurePDFsize_v1([10 10 650 600]);
ax = gca; hold(ax, 'on');
imagesc(ax, rel_t(1:size(d,1)), rel_t(1:size(d,2)), d); 
set(ax, 'YDir', 'reverse');
colormap gray; colorbar(ax);
xline(ax, 0, 'LineStyle', '--', 'LineWidth', 1, 'Color', 'white'); 
xline(ax, durs(di)/fs, 'LineStyle', '--', 'LineWidth', 1, 'Color', 'white'); 
% xlim(ax, [rel_t(1) rel_t(size(d,1))]);
% ylim(ax, [rel_t(1) rel_t(size(d,2))]);
xlim(ax, [rel_t(1) 0.16]);
ylim(ax, [rel_t(1) 0.16]);
title('Mean neural difference', 'FontSize', 14);
fn_pdf = fullfile(fd_save, sprintf('%s.%s_%s.meanDiffMatrix.pdf', birdID, syls{1}, syls{2}));
print(fig, fn_pdf, '-dpdf', '-painters');
...
fn_pdf = fullfile(fd_save, sprintf('%s.%s_%s.pseudoMeanBinary.pdf', birdID, syls{1}, syls{2}));
print(fig, fn_pdf, '-dpdf', '-painters');

% calcualte difference matrix on binarized data
dist_all = nan(size(mean1,2), size(mean2,2));
for tii=1:size(mean1,2)
  for tjj=1:size(mean2,2)
    m1 = mean1(:, tii)>=thre;
