...
% Zhilei, 09/05/2025
% differ from v1: use the masked data
...
fd_data = fullfile(fd_base, 'Figures', pairID, 'AcousticSpace', birdID, 'embed', [vae_run '_decodePopMatrix4'], 'masked');
...
for ii=1:size(syls,2)
  if ismember(syls{ii}, to_mask); continue; end
  for jj=(ii+1):size(syls,2)
    if ismember(syls{jj}, to_mask); continue; end
