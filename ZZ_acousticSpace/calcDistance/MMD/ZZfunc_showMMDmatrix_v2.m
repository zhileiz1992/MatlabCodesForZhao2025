function [ax] = ZZfunc_showMMDmatrix_v2(ax, d, clim, tick_loc, tick_label, title_str, plot_boundary, line_loc)
...
if plot_boundary
  for si=1:length(line_loc)
    ...
    plot(ax, [1 1],  [0 line_loc(1)], 'LineStyle', '--', 'LineWidth', 0.25, 'Color', '#99d8c9');
    if si<length(line_loc)
    stop_x = find(isnan(d(:, line_loc(si)+1)));
    if isempty(stop_x)
      xline(ax, line_loc(si), 'LineStyle', '--', 'LineWidth', 0.25, 'Color', '#99d8c9');
    else
      plot(ax, [line_loc(si) line_loc(si)], [0 stop_x(1)], 'LineStyle', '--', 'LineWidth', 0.25, 'Color', '#99d8c9');
    end
    end
    ...
    if isempty(stop_y)
      ...
      plot(ax, [stop_y(end) size(d,2)], [line_loc(si) line_loc(si)], 'LineStyle', '--', 'LineWidth', 0.25, 'Color', '#99d8c9');
    end
  end
end
