...
% differ from v2: use the renaming where low-quality clusters are identified
clear; close all;
...
suffix = 'dvae_warp';
% where to save
fd_save =  fullfile(fd_home, 'MMD', 'matrixCall3', 'all');
...
for ii=1:size(syls,2)
  for jj=ii:size(syls,2)
    ...
    to_sample = min([size(dvae1,2) size(dvae2,2)]);
